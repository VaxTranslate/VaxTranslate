...
const Translate = () => {
  ...
  const handleFileChange = (event) => {
    ...
    const fileListHeight = selectedFiles.length * 105; // Assuming each item is 105px
    const minHeight = 650; // Minimum height (original)
    const newHeight = Math.max(minHeight, fileListHeight + 300); // Add some buffer space (adjust as needed)
  ...
  return (
    <div className="d-flex align-items-center justify-content-center" style={{marginTop: "50px"}}>
      ...
      >
        <div
      className="card shadow p-5 d-flex flex-column align-items-center flex-grow-1"
      style={{
        width: "100%",
        maxWidth: "800px",
        height: `${cardHeight}px`, // Use dynamic height
        borderRadius: "10px",
      }}
    >
